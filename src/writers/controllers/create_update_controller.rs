...
use crate::writers::{add_new_controller_to_main_rs, write_to_controller_name_html, write_to_controllers_mod, write_to_new_post_controller, write_to_new_update_controller};
use eyre::Error;
use crate::writers;
...
/// ```
pub fn create_update_controller_in_existing_folder(controller_name: String) -> Result<(), Error> {
    ...
    if input == "y" {
        ...
        if !has_controller {
            ...
            if input == "y" {
                ...
                write_to_new_update_controller(controller_name.clone()).unwrap_or_else(|why| {
            ...
            // if the folder does exist, add the controller to the file for that controller
            write_to_new_update_controller(controller_name.clone())
                .unwrap_or_else(|why| {
                    println!("Failed to write to controller: {:?}", why.to_string());
                });

...
"#;
    ...
    let result = create_migration(
        "add_page_id_to_funnel_steps",
        vec!["page_id:integer".to_string()],
    )
    .await;
    assert!(result.is_ok());
    ...
    let migration_dir = migration_dirs
        ...
        .find(|entry| {
            entry
                .file_name()
                .to_string_lossy()
                .contains("add_page_id_to_funnel_steps")
        })
        .unwrap();
...
"#;
    ...
    let result = create_migration(
        "create_users_table",
        vec!["name:string".to_string(), "email:string".to_string()],
    )
    .await;
    assert!(result.is_ok());
